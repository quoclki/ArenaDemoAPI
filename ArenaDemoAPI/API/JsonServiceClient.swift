...
public class JsonServiceClient : ServiceClient
{
    ...
    func handleResponse<T : JsonSerializable>(intoResponse:T, data:Data, response:URLResponse, error:NSErrorPointer = nil) -> T? {
        if let nsResponse = response as? HTTPURLResponse {
            if nsResponse.statusCode >= 400 {
                ...
                let ex = fireErrorCallbacks(error: NSError(domain:self.domain, code:nsResponse.statusCode, userInfo:errorInfo as! [String : Any]))
                if error != nil {
                    error?.pointee = ex
                }
    ...
    public func createRequestDto<T : JsonSerializable>(url:String, httpMethod:String, request:T?) -> NSMutableURLRequest {
        ...
        if let dto = request {
            ...
            if let json = NSString(data: requestBody!, encoding: String.Encoding.utf8.rawValue){
                print("JSON: \(json)")
            }
        }
    ...
    @discardableResult public func send<T : JsonSerializable>(intoResponse:T, request:NSMutableURLRequest) throws -> T {
        ...
        do {
            ...
            var exp: NSError? = ex as NSError
            if let r = response, let e = self.handleResponse(intoResponse: intoResponse, data: data, response: r, error: &exp) {
                return e
            }
...
public class Metadata {
    ...
    static func create(_ superProperties: [PropertyType], selfProperties: [PropertyType]) -> Metadata{
        return Metadata(properties: superProperties + selfProperties)
    }
}
...
extension HasMetadata{
    ...
    public func toJson() -> String {
        ...
        for p in type(of: self).metadata.properties {
...
public extension String
{
    ...
    public func toDouble() -> Double {
        return (self.replacingOccurrences(of: ",", with: "") as NSString).doubleValue
    }
